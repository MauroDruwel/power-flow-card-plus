...
export class PowerFlowCardPlus extends LitElement {
  ...
  setConfig(config: PowerFlowCardPlusConfig): void {
    ...
    if (
      ...
      (!config.entities?.battery?.entity &&
        !config.entities?.battery_2?.entity &&
            ...
            ? html`<div class="row">
                ...
                ${battery2.has
                  ? batteryElement(this, this._config, {
                      battery: battery2,
                      entities: { ...entities, battery: entities.battery_2 },
                    })
                  : nothing}
                ${individualFieldLeftBottom
  ...
  private _computeRenderData() {
    ...
    const createBatteryObject = (field: "battery" | "battery_2", fallbackName: string) => {
      const entityConfig = entities[field];
      const checkIfHasBattery = () => {
        if (!entityConfig?.entity) return false;
        if (typeof entityConfig?.entity === "object")
          return entityConfig?.entity.consumption || entityConfig?.entity.production;
        return entityConfig?.entity !== undefined;
      };
      return {
        entity: entityConfig?.entity,
        has: checkIfHasBattery(),
        mainEntity:
          typeof entityConfig?.entity === "object"
            ? entityConfig.entity.consumption
            : entityConfig?.entity,
        name: computeFieldName(this.hass, entityConfig, fallbackName),
        icon: computeFieldIcon(this.hass, entityConfig, "mdi:battery-high"),
        state_of_charge: {
          state: getBatteryStateOfCharge(this.hass, this._config, field),
          unit: entityConfig?.state_of_charge_unit ?? "%",
          unit_white_space: entityConfig?.state_of_charge_unit_white_space ?? true,
          decimals: entityConfig?.state_of_charge_decimals || 0,
        },
        state: {
          toBattery: getBatteryInState(this.hass, this._config, field),
          fromBattery: getBatteryOutState(this.hass, this._config, field),
          toGrid: 0,
          toHome: 0,
        },
        tap_action: entityConfig?.tap_action,
        hold_action: entityConfig?.hold_action,
        double_tap_action: entityConfig?.double_tap_action,
        color: {
          fromBattery: entityConfig?.color?.consumption,
          toBattery: entityConfig?.color?.production,
          icon_type: undefined as string | boolean | undefined,
          circle_type: entityConfig?.color_circle,
        },
      };
    };
    const battery = createBatteryObject(
      "battery",
      this.hass.localize("ui.panel.lovelace.cards.energy.energy_distribution.battery")
    );
    const battery2 = createBatteryObject("battery_2", localize("editor.battery_2"));
    const home = {
    ...
    battery2.state.fromBattery = adjustZeroTolerance(
      ...
      entities.battery_2?.display_zero_tolerance
    );
    ...
    if (entities.battery_2?.display_zero === false && !hasBattery2Flow) {
      battery2.has = false;
    }
    const combinedBattery = {
      ...battery,
      has: battery.has || battery2.has,
      state: {
        toBattery: (battery.state.toBattery ?? 0) + (battery2.state.toBattery ?? 0),
        fromBattery: (battery.state.fromBattery ?? 0) + (battery2.state.fromBattery ?? 0),
        toGrid: 0,
        toHome: 0,
      },
    };
    ...
    if (combinedBattery.state.fromBattery === 0) {
      combinedBattery.state.toGrid = 0;
      combinedBattery.state.toHome = 0;
    }
    computePowerDistributionAfterSolarAndBattery({
      entities: {
        grid: entities.grid,
        battery: entities.battery,
        battery_2: entities.battery_2,
    ...
    const totalHomeConsumption = Math.max(
      (grid.state.toHome ?? 0) + (solar.state.toHome ?? 0) + (combinedBattery.state.toHome ?? 0),
      0
    );
    const homeBatteryCircumference = combinedBattery.state.toHome
      ? CIRCLE_CIRCUMFERENCE * (combinedBattery.state.toHome / totalHomeConsumption)
      : 0;
    ...
    if (entities.battery_2?.icon !== undefined) battery2.icon = entities.battery_2?.icon;
    const battery2UseMetadataIcon = entities.battery_2?.use_metadata;
    if (battery2UseMetadataIcon) {
      const metadataIcon = computeFieldIcon(this.hass, entities.battery_2, "NO_ICON_METADATA");
      if (metadataIcon !== "NO_ICON_METADATA") {
    ...
    const newDur: NewDur = {
      batteryGrid: computeFlowRate(
        this._config,
        Math.max(grid.state.toBattery ?? 0, combinedBattery.state.toGrid ?? 0, 0),
        totalLines
      ),
      batteryToHome: computeFlowRate(
        this._config,
        combinedBattery.state.toHome ?? 0,
        totalLines
      ),
